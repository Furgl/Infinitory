...
public abstract class PlayerScreenHandlerMixin extends ScreenHandlerMixin implements ScreenHandlerAccessor {
	...
	protected ItemStack transferSlot(PlayerEntity player, int index) {
		...
		if (slot != null && slot.hasStack()) {
			...
			if (index == 0) {
				if (!this.callInsertItem(itemStack2, 9, 45+Utils.ADDITIONAL_SLOTS, true)) 
					return ItemStack.EMPTY;
				slot.onQuickTransfer(itemStack2, itemStack);
			} 
			// craft input -> anywhere
			else if (index >= 1 && index < 5) { 
				if (!this.callInsertItem(itemStack2, 9, 45+Utils.ADDITIONAL_SLOTS, false)) 
					return ItemStack.EMPTY;
			} 
			// armor -> anywhere
			...
			// hotbar -> main inventory
				...
				if (!this.callInsertItem(itemStack2, 9, 36, false) || 
						(Utils.ADDITIONAL_SLOTS > 0 && !this.callInsertItem(itemStack2, 46, 46+Utils.ADDITIONAL_SLOTS, false))) 
					return ItemStack.EMPTY;
			} 
			// anywhere -> anywhere
			else if (!this.callInsertItem(itemStack2, 9, 45+Utils.ADDITIONAL_SLOTS, false)) 
				return ItemStack.EMPTY;
